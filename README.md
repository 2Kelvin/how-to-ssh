# How To SSH, Create Hostnames and Static IPs

## Creating `SSH` key pair and SSHing into other servers
...
```bash
ssh ubuntu_user@ubuntuservervm.lab
```

or

```bash
ssh rocky_user@ubuntuservervm.lab
```

## `Firewall` (firewalld (RHEL/Rocky) and ufw (Ubuntu))
